export const darkTheme = {
  bg: "#0F0F0F", // Very dark background
  bgLight: "#1C1C1C", // Dark background
  primary: "#0074D9", // Primary blue color
  text_primary: "#F2F3F4", // Light text color
  text_secondary: "#b1b2b3", // Secondary text color
  card: "#141414", // Dark card background
  card_light: "#1E1E1E", // Slightly lighter card background
  button: "#0074D9", // Blue button color
  beige: "#F5F5DC", // Beige color for elegance
  white: "#FFFFFF", // White color
  black: "#000000", // Black color
};

export const lightTheme = {
  bg: "#FFFFFF", // White background
  bgLight: "#f0f0f0", // Light gray background
  primary: "#6ACD60", // Primary green color
  text_primary: "#111111", // Dark text color
  text_secondary: "#48494a", // Secondary text color
  card: "#FFFFFF", // White card background
  card_light: "#ffffff", // White card background (same as card)
  button: "#4D9E48", // Green button color
  white: "#FFFFFF", // White color
  black: "#000000", // Black color
};
